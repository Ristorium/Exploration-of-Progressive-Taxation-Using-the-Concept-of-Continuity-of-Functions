...
# matplotlib은 불연속점에서도 x축과 수직 방향으로 선을 그리므로, 불연속점 주변의 값까지 제거함.
...
plt.plot(x, f1, label='누진세 적용 세액(100만원)')
plt.plot(x, g1, label='누진세 비적용 세액(100만원)')
plt.axhline(0)
plt.legend()
plt.title("누진세에 따른 종합소득세 그래프")
plt.show()
...
for a, val in sodeug_list.items():
    plt.vlines(x=a, ymin=0, ymax=val, color='gray', linestyle='--', linewidth=1)
plt.plot(x, f2, label='세후 수익(누진세 적용, 100만원)')
plt.plot(x, g2, label='세후 수익(누진세 비적용, 100만원)')
plt.axhline(0)
plt.legend()
plt.title("누진세에 따른 세후 수익 그래프")
plt.show()